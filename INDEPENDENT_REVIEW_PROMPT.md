...
同时读取 `00_project/governance/modules/engineering/NON_REGRESSION_CONTROL.md` 和 `00_project/governance/NON_REGRESSION_CONTRACT.yaml`。当前 Target 命中 `LOCKED` Invariant 时，独立核验 `python3 09_quality/non_regression/validate_non_regression.py` 的 Target、退出码和证据。必需 Guard、Contract 或输入无效时记录 `REVIEW_NOT_READY`；Guard 确认违反时形成 Finding。框架治理变更不得把 Non-Regression Validation 标记为不适用。
