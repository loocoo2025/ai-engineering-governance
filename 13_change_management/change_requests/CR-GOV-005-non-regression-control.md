...
```text
INITIAL_REVIEW_TARGET: f8b84756f2446804ee0a19761b02d8d2e264b387
INITIAL_REVIEW_RECORD: 05_reviews/C04-GOV-005-v0.1.9-candidate.md
INITIAL_DECISION: CHANGES_REQUESTED
OPEN_FINDING: C04-GOV-005-F001 / S1
REGRESSION_GUARD_DISPOSITION: REQUIRED
```

整改严格限于该 Finding 的关闭条件：Validator 现在核验所有 `FRAMEWORK / PROJECT` scope 的 `SUPERSEDED` 活跃链必须终止于 `LOCKED`，`PROPOSED` 不得接管已生效 Invariant；新增两类 fail-closed 负向测试、合法 `LOCKED` successor 正向测试，以及 `NRC-FWK-005` 永久 Guard。整改完成后必须形成新精确 Commit，并由全新独立 C04 Session 复审。
