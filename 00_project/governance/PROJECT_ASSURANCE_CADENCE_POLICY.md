...
验证 Finding 的关闭由独立 C04 完成；Caller 保存已经形成的正式结论、同步其引用状态和派生索引，不构成再次验证关闭。

仅包含这些记录动作、未改变结论、Finding、关闭条件、精确 Target、授权或治理语义的 Commit，不因归档或状态登记本身再次触发正式 C04。Caller 应检查本次完整 Diff，确认没有混入实质变更，并按以下方式验证归档保真：

1. 收到结果时确定正式记录正文的起止边界，保留可追溯的原始返回内容；聊天前言不属于正文时，记录提取边界。
2. 在编辑前对选定正文计算 SHA-256，归档后按同一字节约定比对。若仅统一换行或末尾空行，必须声明规范化方式并对来源、归档采用同一方式；不得用事后生成的 Hash 冒充事前来源证据。
3. 验证 Review ID、精确 Target、Decision、Finding 状态及引用、索引与来源一致。Hash 只证明内容一致，不能替代对内容真实性或接受条件的判断。
4. 比对失败时先核对来源并修正复制错误；原 Reviewer 内容本身有误时保留原记录，单独记录 Caller Correction，不静默改写结论。

机械结果引用既有 Review Record，并保存在现有变更或审计记录中，无需新增治理文件。原 `PASS` 仍只绑定原精确 Target；后代记录 Commit 不获得新的 `PASS`。如混入实质变更、其他正式 C04 触发事件或项目明确要求，则按本节触发规则重新判断，不能仅凭“登记 Commit”名称豁免。
