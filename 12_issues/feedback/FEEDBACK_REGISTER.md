...
任何实际发现且有可信依据、但尚未完成事实判断的疑问、体验问题、治理缺口、改进建议、疑似缺陷、现场报告或 C04 范围外问题，必须先登记为 FB，再由 C06 分类。不得在收到时直接把它宣称为 BUG、CR 或正式项目决定，也不得因为它不阻断当前任务而丢弃。
...
登记不要求立即处理。项目可以继续推进，只要当前任务没有 Open Finding，且该反馈不命中安全、数据完整性、权限或不可逆风险的强制停止边界。
...
| ID | 反馈摘要 | 与当前任务关系 | 类型 | 优先级 | 状态 | 去向 | 决定 Owner / 责任角色 | 关联记录 / 下一步 |
|---|---|---|---|---|---|---|---|---|
| FB-XXX | | CORE / RELATED_NON_BLOCKING / OUT_OF_SCOPE / UNKNOWN | UNKNOWN | P0/P1/P2/P3 | NEW | UNKNOWN | Human Owner / C06 | |
...
是否需要解决、何时解决、是否接受风险，以及争议项最终是否构成产品问题，由正确的 Human Project Owner / Decision Owner / Risk Owner 裁定。AI 可以给出证据和建议；裁定为 `NO_ACTION / REJECTED / DEFERRED` 时仍保留原始反馈、理由和决定人，防止问题被删除后再次无上下文出现。
